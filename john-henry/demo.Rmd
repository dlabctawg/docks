...
# Data Management: qdpb

This workflow follows a database management convention. We begin with a data quality `dq` stage where problems concerning sample selection and importing can be addressed. We pass these results to a data dump `dd` stage for cleaning and merging with metadata. Next there will be one or more data pipes `dp` to pass our cleaned data to perhaps several method-specific data formats. Finally the data base `db` stage may include analyzed and modeled results allowing you to accomplish your particular research objectives. Separating the workflow into these stages helps keep things organized while letting us intervene at controlled checkpoints to make different research decisions.
